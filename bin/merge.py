...
def merge_files(python_content, js_content, compression=False):
    ...
    if compression:
        return f"""eval(["exec(__import__('zlib').decompress(__import__('base64').b64decode('{str_python_content}')).decode(encoding='utf-8',errors='ignore'))","new Response((await new Blob([Uint8Array.from(atob('{str_js_content}'),c=>c.charCodeAt())]).stream().pipeThrough(new DecompressionStream('deflate')))).text().then(eval)"][1|0==2]);"""
    else:
